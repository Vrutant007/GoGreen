...
const ShowCoupon = () => {
    ...
    const fetchCoupon = async() => {
        const response = await fetch(`${apiUrl}/get-coupon-code`,{
            method: 'GET',
            headers: {
    ...
    const deleteCoupon = async () => {
        if(confirm('Are You Sure you want to Delete !')) {
            const response = await fetch(`${apiUrl}/delete-coupon-code/${id}`,{
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept' : 'application/json',
            ...
            .then(result => {
                if(result.status == 200) {
                    const NewCode = coupon.filter(coupon => coupon.id != id)
                    SetCoupon(NewCode);
        ...
        <div className='container'>
            <div className='row'>
                <div className='d-flex justify-content-between mt-5 pb-3'>
                    ...
                    <Link to='/admin/coupon-code/create' className='btn btn-primary'>Create</Link>
                </div>
                {/*SideBar */}
                                    ...
                                    coupon && coupon.map(coupon => {
                                        return (
                                            <tr>
                                                ...
                                                <td>{coupon.expire_at ? coupon.expire_at : 'No Expiry'}</td>
                                                <td>
                                                    <Link to={`/admin/coupon-code/edit/${coupon.id}`}  className='text-primary'>
                                                    ...
                                                    <Link className='text-danger ms-2' onClick={() => deleteCoupon(coupon.id)}>
