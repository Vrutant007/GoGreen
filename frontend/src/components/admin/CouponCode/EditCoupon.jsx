...
const EditCoupon = () => {
    ...
    const {
        ...
        defaultValues: async () => {
            const response = await fetch(`${apiUrl}/show-coupon-code/${params.id}`,{
                method: 'GET',
                headers: {
            ...
            .then(result => {
                ...
                if (result.status == 200) {
                    ...
                    const formattedDate = result.data.expire_at
                    ? new Date(result.data.expire_at).toISOString().split('T')[0]
                    : '';
    ...
    const saveCoupon = async(data) => {
        const response = await fetch(`${apiUrl}/update-coupon-code/${params.id}`,{
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Accept' : 'application/json',
