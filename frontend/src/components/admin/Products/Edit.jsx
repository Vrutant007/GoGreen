...
const Edit = ({placeholder}) => {
  ...
  const deleteImage = async(id) => {
    ...
    .then(result => {
      if(result.status == 200){
        const newProductImages = productImages.filter(producImage =>producImage.id != id)
  ...
  const changeImage = async(image) => {
    const response = await fetch(`${apiUrl}/change-product-default-image?product_id=${params.id}&image=${image}`,{
      method: 'GET',
      headers: {
          'Content-Type': 'application/json',
          'Accept' : 'application/json',
                  ...
                  <div className='card shadow'>
                      <div className="card-body p-4">
                          ...
                          <div className='mb-3'>
                            <div className='row'>
                              {
                                productImages && productImages.map((producImage,index) => {
                                  return (
                                    <div className='col-md-3' key={`image-${index}`}>
                                      <div className='card shadow'>
                                        <img src={producImage.image_url} alt="" className='w-100'/>
                                        
                                      </div>
                                      <button type='button' className='btn btn-danger mt-3 w-100' onClick={() => deleteImage(producImage.id)}>Delete</button>
                                      <button type='button' className='btn btn-primary mt-3 w-100' onClick={() => changeImage(producImage.image)}>Set as Default</button>
                                    </div>
                                  )
                                })
                              }
