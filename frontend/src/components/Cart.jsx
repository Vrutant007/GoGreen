...
const Cart = () => {
    const { cartData, Delivery, grandTotal, subTotal, updateCartItem, deleteCartItem } = useContext(CartContext)
                            ...
                            <tbody>
                                {
                                    cartData.length == 0 &&
                                    <tr>
                                        <td colSpan="4" className="text-center"><h3>No items in cart</h3></td>
                                    </tr>
                                }
                                {
                                    cartData && cartData.map(item => {
                                        return(
                                            <tr>
                                                ...
                                                <td valign='middle'>
                                                    <input min={1} max={10} style={{width: '100px'}} type="number" value={qty[item.id] || item.qty} className='form-control' onChange={(e) => handleQty(e, item.id)} />
                                                </td>
                                                <td valign='middle'>
